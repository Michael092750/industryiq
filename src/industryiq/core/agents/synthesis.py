...
def _dedup_sources(source_lists: Iterable[list[dict[str, Any]]]) -> list[dict[str, Any]]:
    """Flatten and de-dup citations by their ``source`` label, order preserved."""
    seen: set[str] = set()
...
class Synthesizer:
    ...
    def synthesize(self, plan: Plan, results: Mapping[str, CapabilityResult]) -> RunResult:
        ordered = [
            (node.node_id, results[node.node_id]) for node in plan.nodes if node.node_id in results
        ]
        sources = _dedup_sources(result.sources for _node_id, result in ordered)
        ...
        if not ordered:
            answer = "No results were produced for this request."
